            ...
            parallel{
                stage("Distribute Metaserver"){
                    steps{    
                        ...
                        sh "ssh -v -o StrictHostKeyChecking=no $METASERVER_USER@$METASERVER_HOST -p $METASERVER_PORT \"docker load < $METASERVER_APP-${BRANCH}.tar && mkdir -p ~/$REPO_NAME\""
                ...
                stage("Distribute Metaclient"){
                    steps{    
                        ...
                        sh "ssh -v -o StrictHostKeyChecking=no $METACLIENT_USER@$METACLIENT_HOST -p $METACLIENT_PORT \"docker load < $METACLIENT_APP-${BRANCH}.tar && mkdir -p ~/$REPO_NAME\""
        ...
        stage("Deploy"){
            parallel{
                stage("Deploy Metaserver"){
                    steps{
                        sh "ssh -v $METASERVER_USER@$METASERVER_HOST -p $METASERVER_PORT \"cd ~/$REPO_NAME && docker service update --force ${REPO_NAME}_$METASERVER_APP && docker system prune -a -f\""
                    }
                }
                stage("Deploy Metaclient"){
                    steps{
                        ...
                        sh "ssh -v $METACLIENT_USER@$METACLIENT_HOST -p $METACLIENT_PORT \"cd ~/$REPO_NAME && docker service update --force ${REPO_NAME}_$METACLIENT_APP && docker system prune -a -f\""
                    }
                }
            }
        }
